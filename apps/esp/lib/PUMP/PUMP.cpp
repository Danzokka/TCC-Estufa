...
bool PumpController::begin() {
    ...
    httpServer.on("/pump/activate", HTTP_POST, [this]() { handleActivatePump(); });
    httpServer.on("/pump/deactivate", HTTP_POST, [this]() { handleDeactivatePump(); });
    httpServer.on("/pump/status", HTTP_GET, [this]() { handlePumpStatus(); });
    httpServer.on("/pump/emergency-stop", HTTP_POST, [this]() { handleEmergencyStop(); });
    ...
    if (xSemaphoreTake(pumpMutex, 1000 / portTICK_PERIOD_MS) == pdTRUE) {
        if (doc.containsKey("duration")) {
            ...
            if (validateDuration(duration)) {
                ...
                if (!success) errorMsg = "Failed to activate pump for duration";
            } else {
            ...
            if (validateVolume(volume)) {
                ...
                if (!success) errorMsg = "Failed to activate pump for volume";
            } else {
                errorMsg = "Invalid volume specified";
            }
        } else {
            success = activatePump(); // Manual mode
            if (!success) errorMsg = "Failed to activate pump manually";
        }
...
String PumpController::createStatusResponse() {
    ...
    doc["status"] = (pumpStatus == PUMP_OFF) ? "off" : (pumpStatus == PUMP_ON) ? "on" : "error";
    doc["enabled"] = pumpEnabled;
    doc["mode"] = (currentMode == MODE_MANUAL) ? "manual" : (currentMode == MODE_DURATION) ? "duration" : "volume";
